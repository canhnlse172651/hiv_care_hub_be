...
export class OrderService {
  ...
  async createOrder(data: CreateOrderType): Promise<OrderResponseType> {
    ...
    if (data.patientTreatmentId) {
      ...
      this.logger.log(`✅ [ORDER_SERVICE] Patient treatment validation passed: PatientTreatmentID=${data.patientTreatmentId}`);
    }
  ...
  private generateBankTransferUrl(payment: any, order: any): string {
    ...
    this.logger.log(`💳 [ORDER_SERVICE] Payment details: PaymentID=${payment.id}, TransactionCode=${payment.transactionCode}`);
