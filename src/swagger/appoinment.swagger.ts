...
export const ApiFindAppointmentByUserId = () => {
  return applyDecorators(
    ...
    ApiQuery({ name: 'sortBy', type: 'string', description: 'Sort by', required: false, enum: ['id', 'createdAt', 'updatedAt', 'appointmentTime', 'status', 'type'] }),
    ApiQuery({ name: 'sortOrder', type: 'string', description: 'Sort order', required: false, enum: ['asc', 'desc'] }),
    ApiQuery({ name: 'status', type: 'string', description: 'Status', required: false, enum: ['PENDING', 'CHECKIN', 'PAID', 'PROCESS', 'CONFIRMED', 'COMPLETED', 'CANCELLED'] }),
    ApiQuery({ name: 'type', type: 'string', description: 'Appointment Type', required: false, enum: ['ONLINE', 'OFFLINE'] }),
    ApiQuery({ name: 'dateFrom', type: 'string', format: 'date-time', description: 'Filter from date (appointmentTime >= dateFrom)', required: false, example: '2024-03-20T00:00:00Z' }),
    ApiQuery({ name: 'dateTo', type: 'string', format: 'date-time', description: 'Filter to date (appointmentTime <= dateTo)', required: false, example: '2024-03-21T00:00:00Z' }),
    ApiResponse({
      status: 200,
